...
const WriteRoom = () => {
    ...
    const checkClick = (id) => {
        ...
        // })
        // console.log(AmenityText)
        // console.log(formData.r_amenity)
    }
    // useEffect(()=>{
    //     const AmenityText = (amenitys.filter(am=>am.checked).map(am=>am.text).join("*"))
    //     setFormData({
    ...
    //변경되는 파일을 서버로 업로드 전송하기
    const onChangeImage = (e) =>{
        const {name} = e.target;
        console.log(name)
        const imageFormData = new FormData();
        // 폼태그에 데이터추가하기
        imageFormData.append('img',e.target.files[0]); // key="img" value=e.target.files[0]
        //전송
            ...
            <form onSubmit={onSubmit}>
                <table className='defaulttable'>
                    <tbody>
                        <tr>
                            <td>방이름</td>
                            <td><input type="text" name="r_name" value={FormData.r_name} onChange={onChange}/></td>
                        </tr>
                        <tr>
                            <td>베드타입</td>
                            <td>
                                <select name = 'r_bed' onChange={onChange} value = {formData.r_bed}>
                                    ...
                                    <option value='싱글 베드 1개 & 세미 더블 배드 1개'>싱글 베드 1개 & 세미 더블 배드 1개</option>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="checkbox" value={amenity.text} 
                                checked={amenity.checked} onChange={()=>checkClick(amenity.id)}/></div>)}
                            </td>
                        </tr>
                        <tr>
                            <td>이용요금</td>
                            <td>
                                <input type="number" min="50000" step={1000} name="r_price" value={FormData.r_price} onChange={onChange}/>
                            </td>
                        </tr>
                        <tr>
                            <td>이미지1</td>
                            <td><input type="file" name="r_img1" onChange={onChangeImage}/>
                            {formData.r_img1 && <div><img src={`${API_URL}/upload/event/${formData.r_img1}`} width="100px" alt=''/></div>}</td>
                        </tr>
                        <tr>
                            <td>이미지2</td>
                            <td><input type="file" name="r_img2" onChange={onChangeImage}/>
                            {formData.r_img2 && <div><img src={`${API_URL}/upload/event/${formData.r_img2}`} width="100px" alt=''/></div>}</td>
                        </tr>
                        <tr>
                            <td>이미지3</td>
                            <td><input type="file" name="r_img3" onChange={onChangeImage}/>
                            {formData.r_img3 && <div><img src={`${API_URL}/upload/event/${formData.r_img3}`} width="100px" alt=''/></div>}</td>
                        </tr>
                        <tr>
                            <td>이미지4</td>
                            <td><input type="file" name="r_img4" onChange={onChangeImage}/>
                            {formData.r_img4 && <div><img src={`${API_URL}/upload/event/${formData.r_img4}`} width="100px" alt=''/></div>}</td>
                        </tr>
                        <tr>
                            <td>상세설명글</td>
                            <td>
                                <textarea name="r_desc" value = {formData.r_desc} onChange={onChange}></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td colSpan={2}>
